...
pub fn unpack(table: Vec<BitVector>, nb_qubits: usize) -> Vec<Vec<u8>> {
    if table.is_empty() {
        return Vec::new();
    }
    
    let num_cols = table.len();
    let mut result = vec![vec![0u8; num_cols]; nb_qubits];
    
    for (col_idx, bv) in table.into_iter().enumerate() {
        for row_idx in 0..nb_qubits {
            result[row_idx][col_idx] = if bv.get(row_idx) { 1u8 } else { 0u8 };
        }
    }
...
pub fn pack(table: Vec<Vec<u8>>) -> Vec<BitVector> {
    if table.is_empty() {
        return Vec::new();
    }
    
    let nb_qubits = table.len();
...
pub fn optimal_synthesis(table: Vec<BitVector>, nb_qubits: usize) -> Vec<BitVector> {
    let mut table = unpack(table, nb_qubits);
    let mut t_count = table[0].len() - 1;

    while let Some(t) = solve(table.clone(), t_count) {
        println!("successfully synthesised with {} T-gates", t_count);
        t_count -= 1;
        table = t; 
    }
...
pub fn solve(table: Vec<Vec<u8>>, t_count: usize) -> Option<Vec<Vec<u8>>> {
    let num_rows = table.len();
    let num_clauses_estimate = num_rows * num_rows * num_rows * t_count;
    
    println!("Solving SAT instance: {} qubits, {} T-gates, ~{} clauses", num_rows, t_count, num_clauses_estimate);
    
    let mut s = Solver::new();
    s.set_num_threads(10);

    let vars: Vec<Vec<Lit>> = (0..table.len())
        .map(|_| (0..t_count).map(|_| s.new_var()).collect())
        .collect();

    for i in 0..table.len() {
        for j in i..table.len() {
            for k in j..table.len() {
                // Calculate parity of i & j & k
                let and_vec : Vec<u8> = table[i].iter()
                    .zip(&table[j])
                    .zip(&table[k])
                    .map(|((&ti, &tj), &tk)| ti & tj & tk)
                    .collect();

                let parity = and_vec.iter().copied().fold(0u8, |acc, x| acc ^ (x & 1)) == 1;

                // Build auxiliaries t_c = vars[i][c] & vars[j][c] & vars[k][c]
                let mut and_vars: Vec<Lit> = Vec::with_capacity(vars[i].len());
                for c in 0..vars[i].len() {
                    let x = vars[i][c];
                    let y = vars[j][c];
                    let z = vars[k][c];
                    let t = s.new_var();
                    ...
                    s.add_clause(&[!t, z]);
                    // (x & y & z) -> t  ===  (!x ∨ !y ∨ !z ∨ t)
                    s.add_clause(&[!x, !y, !z, t]);
                    and_vars.push(t);
                }
                // set to same parity as input table
                s.add_xor_literal_clause(&and_vars[..], parity);
            }
        }
    }
    ...
    let solution: Vec<Vec<u8>> = vars
        .iter()
        .map(|row| {
            row.iter()
                .map(|&lit| if s.is_true(lit) { 1u8 } else { 0u8 })
                .collect()
        })
        .collect();
