...
fn main() {
    ...
    let do_optimal = args.iter().any(|s| s.to_lowercase().ends_with("optimal"));

    if !(do_bb_merge || do_fast_t_merge || do_internal_h_opt || do_tohpe || do_fast_todd || do_optimal) {
    ...
    println!("File {} processed\n", filename);
     {
        ...
        if do_tohpe || do_fast_todd || do_optimal { println!("Internal Hadamard gates gadgetization"); c = c.hadamard_gadgetization(); }
        ...
        if do_optimal { println!("Running optimal synthesis"); c =  c.t_opt("optimal".to_string()); }
